...
G4VPhysicalVolume* A2CryoTarget::Construct1(G4LogicalVolume *MotherLogic, G4double Z0){
  ...
  G4double l_trgt = (14.63 + 4 + 9.721)*cm; //! See target manual
  G4double r_trgt = 4.1*cm;
  //c Outer flange parameters
  G4double l_flangeo = 0.26*cm;
  G4double t_flangeo = 1.1*cm;
 ...
 //c Parameters for the tmat supply tube
  ...
  G4double t_KAPT = fType==kCryoHe ? 0.0175*cm : 0.0125*cm;//   ! Valid for all kapton parts
  ...
  //CFK tube + window
  /////////////////////
  G4Tubs* CFKT=new G4Tubs("CFKT",r_CFK-t_CFK,r_CFK,l_CFK/2,0*deg,360*deg);
  ...
  G4Tubs* FLA1=new G4Tubs("FLA1",r_flangeo - t_flangeo,r_flangeo,l_flangeo/2.,0*deg,360*deg);
  ...
  G4Tubs* FLA2=new G4Tubs("FLA2",r_flangei - t_flangei,r_flangei,l_flangei/2.,0*deg,360*deg);
  ...
  char foilMat[16];
  if (fType==kCryoHe) strcpy(foilMat, "G4_MYLAR");
 ...
 new G4PVPlacement(0,G4ThreeVector(0,0, -l_trgt/2. + l_CU64 + l_lD2A + t_lD2B - r_lD2B),KAPBLogic,"KAPB",fMyLogic,false,6);
  ...
  //LH2 or LD2
  ///////////////////////////////
    G4Tubs* LD2A=new G4Tubs("LD2A",0,r_lD2A,l_lD2A/2.,0*deg,360*deg);
    ...
    G4Tubs* LD2D=new G4Tubs("LD2D",r_lD2D - t_lD2D,r_lD2D,l_lD2D/2.,0*deg,360*deg);
  ...
  //Copper tube
  ////////////////////////////
  G4Tubs* CU64=new G4Tubs("CU64",r_CU64 - t_CU64,r_CU64,l_CU64/2.,0*deg,360*deg);
